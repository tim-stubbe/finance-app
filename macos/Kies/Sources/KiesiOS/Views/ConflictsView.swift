...
/// Liste offener Sync-Konflikte (siehe SyncEngine.pushOutbox/SyncConflict) -
/// bewusst kein automatischer Merge: pro Konflikt entscheidet der Nutzer
/// zwischen "Server behalten" (verwirft die eigene Änderung) und "Meine
/// Version behalten" (überschreibt den Server beim nächsten Sync
/// unbedingt). Erreichbar über das Banner in TodayView, sobald welche
/// vorliegen.
struct ConflictsView: View {
                ...
                ForEach(engine.conflicts) { conflict in
                    Section {
                        VStack(alignment: .leading, spacing: 6) {
                            Text(kindLabel(conflict.entity_type))
                                .font(.headline)
                            Text(reasonText(conflict))
                                .font(.subheadline)
    ...
    private func reasonText(_ conflict: SyncConflict) -> String {
        ...
        case "server_newer":
            return "Wurde zwischenzeitlich auch auf dem Server geändert (z.B. über die Web-App)."
        default:
