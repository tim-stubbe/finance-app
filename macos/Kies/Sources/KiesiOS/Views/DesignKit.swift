...
// Light only. Alpine identity is a controlled top-layer watermark, never a
// layout-driving image. This is important: background artwork must never
// influence the SwiftUI content size.
...
enum KColor {
    ...
    static let accentStrong = Color(hex: 0x82C800)
    static let accentInk = Color(hex: 0x172000)
...
/// Fixed visual background. GeometryReader constrains the image to the actual
/// viewport so it can never create the enormous blank vertical space that a
/// free `scaledToFill()` image can introduce in a ZStack.
        ...
        GeometryReader { proxy in
            ZStack(alignment: .top) {
                ...
                Image("AlpenBackground")
                    .resizable()
                    .scaledToFill()
                    .frame(width: proxy.size.width, height: min(300, proxy.size.height * 0.34))
                    .clipped()
                    .opacity(opacity)
                    .mask(
                        LinearGradient(
                ...
                LinearGradient(
                    colors: [Color.white.opacity(0.08), KColor.background.opacity(0.18), KColor.background.opacity(0.95)],
                    startPoint: .top,
                    endPoint: .bottom
                )
                .frame(height: min(320, proxy.size.height * 0.38))
            }
...
struct AlpenBackdrop: View {
    var body: some View { NeonBackdrop(opacity: 0.13) }
}
...
struct KScreen<Content: View>: View {
    ...
    var body: some View {
        ...
        .background(NeonBackdrop(opacity: 0.13))
    }
}
...
struct KSurface<Content: View>: View {
    ...
    var body: some View {
        content
            .padding(padding)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(KColor.surface.opacity(0.985), in: RoundedRectangle(cornerRadius: KRadius.md, style: .continuous))
            .overlay(RoundedRectangle(cornerRadius: KRadius.md, style: .continuous).stroke(KColor.divider.opacity(0.9), lineWidth: 1))
            .shadow(color: Color.black.opacity(0.035), radius: 16, x: 0, y: 7)
    }
}
...
struct KRow: View {
    ...
    var body: some View {
        HStack(spacing: KSpacing.md) {
            ...
            if let trailing {
                Text(trailing)
                    .font(KFont.row.weight(.semibold))
